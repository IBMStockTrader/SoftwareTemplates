...
This service manages user *my-apps*.  The data is backed by a CouchDB-complient document database (Cloudant, CouchDB).  The following operations are available:

`GET /` - gets all MyApps.

`POST /{owner}` - creates a new MyApp for the specified owner.

`GET /{owner}` - gets details for the specified MyApp.

`PUT /{owner}?total={total}` - updates the MyApp total for the specified owner (by adding a stock).

`DELETE /{owner}` - removes the MyApp for the specified owner.

`POST /{owner}/feedback` - submits feedback (to the Watson Tone Analyzer)

All operations return *JSON*.  An *MyApp* object contains fields named *owner*, *total*, *loyalty*, *balance*,
...
of this MyApp. This is called via REST. `POST`ing feedback also makes a REST call to Watson to determine the sentiment and, potentially,
...
To build `MyApp` and run in dev mode, locally, clone this repo and run:
```bash
mvn quarkus:dev
```
...
```bash
docker run -i --rm -p 9080:9080 <your repository>/my-app 
```
